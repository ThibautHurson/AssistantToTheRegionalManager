...
async def authorize(session_token: str = None):
    ...
    try:
        ...
        auth_logger.log_debug("Checking credentials", {"session_token": session_token, "user_email": user.email})
        ...
        if not auth_url:
            auth_logger.log_info("Already OAuth authenticated (from get_authorization_url)", {"user_email": user.email})
            return {"message": "Already OAuth authenticated"}
        
        auth_logger.log_info("Returning auth URL", {"user_email": user.email, "auth_url": auth_url})
